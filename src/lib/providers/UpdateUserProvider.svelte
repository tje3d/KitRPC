<script lang="ts">
	...
	type RequestParams = RouterInputs['users']['updateUser'];
	type ResponseData = RouterOutputs['users']['updateUser'];
	...
	const { clearError, errorMessage, loading, request, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).users.updateUser.mutate(input);
		})
	);

	// Actions
	export function updateUser(input: RequestParams) {
		request(input);
	}
