<script lang="ts">
	...
	type RequestParams = RouterInputs['roles']['createRole'];
	type ResponseData = RouterOutputs['roles']['createRole'];
	...
	const { clearError, errorMessage, loading, request, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).roles.createRole.mutate(input);
		})
	);

	// Actions
	export function createRole(input: RequestParams) {
		request(input);
	}
