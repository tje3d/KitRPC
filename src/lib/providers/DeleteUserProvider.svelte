<script lang="ts">
	...
	type RequestParams = RouterInputs['users']['deleteUser'];
	type ResponseData = RouterOutputs['users']['deleteUser'];
	...
	const { clearError, errorMessage, loading, request, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).users.deleteUser.mutate(input);
		})
	);

	// Actions
	export function deleteUser(input: RequestParams) {
		request(input);
	}
