<script lang="ts">
	...
	type RequestParams = RouterInputs['roles']['deleteRole'];
	type ResponseData = RouterOutputs['roles']['deleteRole'];
	...
	const { clearError, errorMessage, loading, request, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).roles.deleteRole.mutate(input);
		})
	);

	// Actions
	export function deleteRole(input: RequestParams) {
		request(input);
	}
