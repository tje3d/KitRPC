<script lang="ts">
	...
	type RequestParams = RouterInputs['wallet']['createWalletAddress'];
	type ResponseData = RouterOutputs['wallet']['createWalletAddress'];

	// Props
	export let onSuccess: ((data: ResponseData | undefined) => void) | undefined = undefined;
	export let onError: ((error: string) => void) | undefined = undefined;

	// Request for creating a wallet address
	const { clearError, errorMessage, loading, trigger, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).wallet.createWalletAddress.mutate(input);
		})
	);

	// Actions
	export function createWalletAddress(input: RequestParams) {
		trigger.next(input);
	}
