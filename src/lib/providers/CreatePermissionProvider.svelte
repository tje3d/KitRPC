<script lang="ts">
	...
	type RequestParams = RouterInputs['permissions']['createPermission'];
	type ResponseData = RouterOutputs['permissions']['createPermission'];
	...
	const { clearError, errorMessage, loading, request, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).permissions.createPermission.mutate(input);
		})
	);

	// Actions
	export function createPermission(input: RequestParams) {
		request(input);
	}
