<script lang="ts">
	...
	type RequestParams = RouterInputs['users']['createUser'];
	type ResponseData = RouterOutputs['users']['createUser'];
	...
	const { clearError, errorMessage, loading, request, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).users.createUser.mutate(input);
		})
	);

	// Actions
	export function createUser(input: RequestParams) {
		request(input);
	}
