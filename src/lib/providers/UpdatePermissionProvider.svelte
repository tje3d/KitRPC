<script lang="ts">
	...
	type RequestParams = RouterInputs['permissions']['updatePermission'];
	type ResponseData = RouterOutputs['permissions']['updatePermission'];
	...
	const { clearError, errorMessage, loading, request, responseSuccess } = useTrpcRequest(
		createTrpcRequestFn((input: RequestParams) => {
			return trpc(page).permissions.updatePermission.mutate(input);
		})
	);

	// Actions
	export function updatePermission(input: RequestParams) {
		request(input);
	}
